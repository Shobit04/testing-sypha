...
// Start audio-only capture for voice input
async function startAudioOnlyCapture() {
    try {
        console.log('Starting audio-only capture for voice input...');
        
        const audioMode = localStorage.getItem('audioMode') || 'speaker_only';
        
        if (isMacOS) {
            // On macOS, use SystemAudioDump for audio
            const audioResult = await ipcRenderer.invoke('start-macos-audio');
            if (!audioResult.success) {
                throw new Error('Failed to start macOS audio capture: ' + audioResult.error);
            }
            console.log('macOS audio capture started for voice input');
        } else if (isLinux) {
            // Linux - try to get system audio via getDisplayMedia
            try {
                ...
                console.log('Linux system audio capture started for voice input');
                setupLinuxSystemAudioProcessing(audioStream);
            } catch (error) {
                console.warn('System audio failed, trying microphone only:', error);
                await startMicrophoneCapture();
            }
        } else {
            // Windows - use microphone capture
            await startMicrophoneCapture();
        }
        
        return true;
    } catch (error) {
        console.error('Failed to start audio-only capture:', error);
        return false;
    }
}
...
async function startMicrophoneCapture() {
    try {
        ...
        console.log('Microphone capture started for voice input');
...
function stopAudioOnlyCapture() {
    try {
        console.log('Stopping audio-only capture...');
        
        // Stop macOS audio if running
        if (isMacOS) {
        ...
        console.log('Audio-only capture stopped');
        return true;
    } catch (error) {
        console.error('Error stopping audio-only capture:', error);
        return false;
    }
}
...
ipcRenderer.on('update-transcription', (event, transcription) => {
    ...
    // You can add UI elements to display the transcription if needed
});
...
const syphaCApp = document.querySelector('sypha-c-app');
...
const cheddar = {
    ...
    getCurrentView: () => syphaCApp.currentView,
    getLayoutMode: () => syphaCApp.layoutMode,

    // Status and response functions
    setStatus: text => syphaCApp.setStatus(text),
    setResponse: response => syphaCApp.setResponse(response),
