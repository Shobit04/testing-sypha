...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\sypha-c-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'sypha-c-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/sypha-c-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'sypha-c-config');
    } else {
        // Linux and others: ~/.config/sypha-c-config
        configDir = path.join(os.homedir(), '.config', 'sypha-c-config');
    }
