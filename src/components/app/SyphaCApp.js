...
export class SyphaCApp extends LitElement {
    static styles = css`
        * {
    ...
    async handleStart() {
        // Initialize local AI instead of Gemini
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            const success = await ipcRenderer.invoke('initialize-local-ai', localStorage.getItem('customPrompt') || '', this.selectedProfile, this.selectedLanguage);
            ...
            if (success) {
                ...
                this.setStatus('Failed to initialize local AI session');
            }
        }
    }
    ...
    async handleAPIKeyHelp() {
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            await ipcRenderer.invoke('open-external', 'https://github.com/Shobit04/sypha_c');
        }
    }
...
customElements.define('sypha-c-app', SyphaCApp);
