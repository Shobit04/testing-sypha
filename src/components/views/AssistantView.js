...
export class AssistantView extends LitElement {
    ...
    static properties = {
        ...
        lastVoiceQuestion: { type: String },
        lastVoiceAnswer: { type: String },
    };
    ...
    constructor() {
        ...
        this._savedResponses = new Set();
        this.selectedLanguage = 'en';
        this.lastVoiceQuestion = '';
        this.lastVoiceAnswer = '';
    }
    ...
    disconnectedCallback() {
        ...
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            ipcRenderer.removeAllListeners('update-transcription');
        }
    }
    ...
    async startAudioRecording() {
        try {
            ...
            // Start audio capture
            if (window.cheddar && window.cheddar.startAudioOnlyCapture) {
                await window.cheddar.startAudioOnlyCapture();
                console.log('Audio recording started');
            } else {
                console.error('Audio capture not available');
                this.isRecording = false;
                this.requestUpdate();
            }
        } catch (error) {
    ...
    async stopAudioRecording() {
        try {
            ...
            // Stop audio capture
            if (window.cheddar && window.cheddar.stopAudioOnlyCapture) {
                window.cheddar.stopAudioOnlyCapture();
                console.log('Audio recording stopped');
            }

            // If we have transcription, send it to AI
            if (this.currentTranscription.trim()) {
                this.onSendText(this.currentTranscription);
                this.currentTranscription = '';
                this.requestUpdate();
            }
        } catch (error) {
    ...
    render() {
        ...
        return html`
            <div class="response-container" id="responseContainer">
                ${this.lastVoiceQuestion ? html`<div style="margin-bottom: 8px;"><b>Question:</b> ${this.lastVoiceQuestion}</div>` : ''}
                ${this.lastVoiceAnswer ? html`<div style="margin-bottom: 8px;"><b>Answer:</b> ${this.lastVoiceAnswer}</div>` : ''}
            </div>

            ${this.currentTranscription ? html`
                <div class="transcription-display">
                    🎤 <strong>Transcription:</strong> ${this.currentTranscription}
                </div>
            ` : ''}

            <div style="margin-bottom: 10px; display: flex; align-items: center; gap: 10px;">
                <label for="lang-select">Language:</label>
                <select id="lang-select" @change=${e => this.selectedLanguage = e.target.value}>
                    <option value="en" ?selected=${this.selectedLanguage === 'en'}>English</option>
                    <option value="hi" ?selected=${this.selectedLanguage === 'hi'}>Hindi</option>
                </select>
                <button
                    class="audio-button ${this.isRecording ? 'recording' : ''}"
                    @click=${this.handleVoiceAsk.bind(this)}
                ...
                <span style="margin-left: 10px; color: #aaa; font-size: 13px;">Ask by voice</span>
            </div>

            <div class="text-input-container">
                <button class="nav-button" @click=${this.navigateToPreviousResponse} ?disabled=${this.currentResponseIndex <= 0}>
                    <?xml version="1.0" encoding="UTF-8"?><svg
                        width="24px" height="24px" stroke-width="1.7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="#ffffff">
                ...
                >
                    <?xml version="1.0" encoding="UTF-8"?><svg
                        width="24px" height="24px" stroke-width="1.7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M5 20V5C5 3.89543 5.89543 3 7 3H16.1716C16.702 3 17.2107 3.21071 17.5858 3.58579L19.4142 5.41421C19.7893 5.78929 20 6.29799 20 6.82843V20C20 21.1046 19.1046 22 18 22H7C5.89543 22 5 21 5 20Z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"></path>
    ...
    async handleVoiceAsk() {
        ...
        try {
            ...
            if (data.error) {
                this.lastVoiceQuestion = '';
                this.lastVoiceAnswer = 'Error: ' + data.error;
            } else {
                this.lastVoiceQuestion = data.question;
                this.lastVoiceAnswer = data.answer;
            }
        } catch (e) {
            this.lastVoiceQuestion = '';
            this.lastVoiceAnswer = 'Error: ' + e.message;
        }
