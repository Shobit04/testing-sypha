...
const { setupLocalAIIpcHandlers, stopMacOSAudioCapture, sendToRenderer } = require('./utils/localAI');
const { initializeRandomProcessNames } = require('./utils/processRandomizer');
const { applyAntiAnalysisMeasures } = require('./utils/stealthFeatures');
const { getLocalConfig, writeConfig } = require('./config');

const localAISessionRef = { current: null };
let mainWindow = null;
...
function createMainWindow() {
    mainWindow = createWindow(sendToRenderer, localAISessionRef, randomNames);
    return mainWindow;
}
...
function setupGeneralIpcHandlers() {
    // No-op handler for send-image-content to prevent errors
    ipcMain.handle('send-image-content', async (event, imageData) => {
        // You can implement image handling logic here if needed
        return { success: true };
    });
    ...
    ipcMain.on('update-keybinds', (event, newKeybinds) => {
        if (mainWindow) {
            updateGlobalShortcuts(newKeybinds, mainWindow, sendToRenderer, localAISessionRef);
        }
    });
